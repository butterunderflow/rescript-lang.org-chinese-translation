...
一些JS值，例如`setTimeout`，存在于全局范围内。你可以像这样绑定到它们：
...
- 接受一个以`unit`为参数并返回`unit`的函数（它在JS端变成了一个不接受任何内容并且不返回任何内容的函数，也就是 `undefined`）
...
由于`external`是内联的，我们最终得到的生成的JS与手写JS一样可读。
...
像`__filename`和`__DEV__`这样的全局值并不总是存在；您甚至不能将它们建模为`选项(option)`，因为仅在ReScript中引用它们（然后编译为JS）就会触发常见的`Uncaught ReferenceError: __filename is not defined`错误，例如浏览器环境。
