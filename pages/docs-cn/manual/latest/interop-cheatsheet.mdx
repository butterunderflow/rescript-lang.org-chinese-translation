...
# Interop Cheatsheet |  交互速查表

This is a glossary with examples. All the features are described by later pages.
这是一个带有示例的专业词汇表。全部功能都会在接下来的页面中进行描述。


## List of Decorators ｜ 装饰器列表
...
> **注意:** 在ReScript 8.3版本之前, 全部属性（attributes）都是以 `bs.` 作为前缀的. 在新的版本中，则不再需要使用该前缀，格式化工具会自动删除该它。
...
- `@as`: [示例1](attribute#usage), [示例2](bind-to-js-function#fixed-arguments), [示例3](bind-to-js-function#constrain-arguments-better) and [示例4](generate-converters-accessors#usage-3)
- [`@deriving`](generate-converters-accessors#generate-functions--plain-values-for-variants)
- [`@get`](bind-to-js-object#bind-using-special-bs-getters--setters)
- [`@get_index`](bind-to-js-object#bind-using-special-bs-getters--setters)
<!-- - `@ignore` -->
...
- [`@optional`](generate-converters-accessors#optional-labels)
- [`@return`](bind-to-js-function#function-nullable-return-value-wrapping)
- `@send`: [示例1](bind-to-js-function#object-method) and [示例2](pipe#js-method-chaining)
- [`@scope`](bind-to-global-js-values#global-modules)
- [`@set`](bind-to-js-object#bind-using-special-bs-getters--setters)
- [`@set_index`](bind-to-js-object#bind-using-special-bs-getters--setters)
- [`@variadic`](bind-to-js-function#variadic-function-arguments)
...
- [`genType`](https://github.com/reason-association/genType)
- [`@JSX`](jsx)
- `@react.component`: [示例1](/docs/react/latest/introduction) and [示例2](https://github.com/reasonml/reason-react)
- [`@warning`](attribute#usage)
- [`@unboxed`](unboxed)
...
## Global Value ｜ 全局值（Global Value）绑定
...
## Global Module's Value ｜ 全局模块的值 （Global Module's Value）绑定
...
我们可以通过使用 `Js.Nullable.t` 来处理那些可能会为 `undefined` 和 `null`的值：
...
- [绑定JS对象为record](bind-to-js-object#bind-to-record-like-js-objects).
- [绑定JS的散列表对象](bind-to-js-object#bind-to-hash-map-like-js-object).
- [绑定JS的类](bind-to-js-object#bind-to-a-js-object-thats-a-class).

## Function ｜ 函数

### Object Method & Chaining ｜ 对象方法与链式调用
