...
变体类型常受限于一组固定的构造函数. 在极少数情况下您仍然希望能够在已经初始化声明类型以后继续给变体类型添加构造函数. 为此, 我们提供了可扩展变体 ( Extensible Variant ) .

## Definition and Usage | 定义与使用
...
在上面的类型声明中我们用 `..` 定义了一个可扩展变体 `type t`. 使用 `+=` 操作符为这个类型添加构造函数.

**Note:** Don't forget the leading `type` keyword when using the `+=` operator!

**注意:** 在使用 `+=` 操作符时不要忘记开头的 `type` 关键字.

## Pattern Matching Caveats | 模式匹配警告
...
**一个有趣的实事:** 在 ReScript 中, [异常](./exception) 在底层中实际上是可扩展变体, 因此 `exception UserError(string)` 与 `type exn += UserError(string)` 等价. 这是极少数可扩展变体的有意义的用例之一.
...
我们通常建议尽可能坚持地使用[普通变体](./variant)以获得穷尽模式匹配的好处.