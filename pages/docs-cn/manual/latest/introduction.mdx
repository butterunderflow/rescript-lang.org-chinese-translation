...
- TypeScript （诚然高贵）的目标是覆盖整个 JavaScript 特性集，甚至更多。**ReScript 只涵盖了一个精心挑选的 JavaScript 子集**。例如，我们强调简单的 _数据+函数_ 而不是类，强调干净的[模式匹配](pattern-matching-destructuring.md)而不是脆弱的的 `if` 和动态分发，强调[适当的数据建模](variant.md)而不是滥用字符串，等等。JavaScript 超集只会越来越大；ReScript 则不会。\*
