...
JSX 是一个语法糖，允许我们以类似 HTML 的方式组合 React 组件。要使用 JSX，组件需要遵循某些接口约定。本节会深入 JSX 转换的细节，以及 JSX 底层使用的 React API。
...
ReScript React 组件需要是一个（子）模块，有被 JSX 使用的 `make` 和 `makeProps` 函数。为了提高开发效率，我们提供一个 `@react.component` 装饰器来为你创建这些函数：
...
**注意：** `makeProps` 函数总会包含一个 `~key` prop。
...
就像上面的被扩展后的输出，装饰器转换传给 `React.forwardRef` 的函数，就像转换典型的组件 `make` 函数一样。它也会创建一个 `makeProps` 函数，而且会添加 `ref` prop，便于我们在 JSX 调用时使用（`<FancyInput ref=.../>`）。
...
如你所见，它使用 `Friend.make` 和 `Friend.makeProps` 调用 `React.createElement` API。如果你提供了子元素，它会调用 `React.createElementVariadic`（只是 `React.createElement` 的一个不同绑定）：
...
注意，`~children=React.null` 属性没有意义，因为 React 只关心作为第三个参数传递的子元素数组。
