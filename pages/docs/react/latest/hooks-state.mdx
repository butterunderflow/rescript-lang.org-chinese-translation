...
初始渲染过程中，被返回的 `state` 和作为第一个参数传给 `React.useState` 的 initialState 一样。
...
`setState` 函数也可以传给其他组件，对于通过子组件设置父组件的状态非常有用。
...
这个例子中，我们正在创建 `ThemeContainer` 组件，用于管理 `darkmode` boolean 状态，以及将 `setDarkmode` 函数传给 `ControlPanel` 组件来触发状态变更。
...
请注意当 `setDarkmode` 返回一个新值时（例如 `true` 切换到 `false`），将会导致 `ThemeContainer` 的 `className` 进行重渲染，以及嵌套的 `ControlPanel` 的文本切换。
