...
在 React 组件中有两种常见的副作用操作：需要清理的和不需要清理的。我们将在稍后的例子中研究它们的区别，但首先让我们看看接口是什么样的。
...
`React.useEffect` 接收一个函数，包含命令式的，可能有副作用的代码，然后返回一个值 `option<unit => unit>` 作为潜在的清理函数。
...
`useEffect` 调用可能会接收附加的依赖项数组(参见 `React.useEffect1` / `React.useEffect2...7`)。只要提供的依赖项之一发生更改，就会运行 effect 函数。关于这些接口的用处请看 [下面](#effect-依赖).
...
**注意：**你可能好奇为什么 `React.useEffect1` 接收一个 `array`，但像 `useEffect2` 需要一个依赖列表的 `tuple`（例如 `(prop1, prop2)`）。因为元组可以存储不同类型的多个值，数组只能存储一个特定类型的值。可以用 `React.useEffect1(fn, [1, 2])` 模仿 `useEffect2`，另一方面类型检查器不会允许 `React.useEffect1(fn, [1, "two"])` 通过。
...
有时，我们想要在 React 更新完 DOM 之后运行一些额外的代码。网络请求，手动操作 DOM以及记录日志是常见的不需要清理的 effects。因为运行它们之后就可以不管了。

<!-- As an example, let's write a counter component that updates `document.title` on every render: -->
下面是一个计数器组件的例子，每次渲染时更新 `document.title`：
...
我们想要 effects 依赖于 `count`，我们可以只使用下面的 `useEffect` 调用：
...
上面我们研究了如何表达不需要任何清理的副作用。然而，某些 effects 需要进行清理。例如，我们想要订阅一些外部数据源。在这种情况下，清理很重要，否则可能会产生内存泄露！
...
在某些情况下，在每次渲染时进行清理或应用 effect 会产生性能问题。让我们看一个具体的例子，看看 `useEffect` 做了什么：
...
这里，我们将 `[count]` 作为依赖传给 `useEffect1`。这是什么意思？如果 `count` 的值是 5，我们的组件重新渲染时 `count` 的值依然是 5，React 会比较前次渲染的 `[5]` 和之后渲染的 `[5]`。因为数组中所有项是相同的（5 === 5），React 会跳过 effect 函数。这就是优化。

<!-- When we render with count updated to 6, React will compare the items in the `[5]` array from the previous render to items in the `[6]` array from the next render. This time, React will re-apply the effect because `5 !== 6`. If there are multiple items in the array, React will re-run the effect even if just one of them is different. -->
当我们使用更新为 6 的 `count` 进行渲染时，React 会比较前次渲染的 `[5]` 和之后渲染的 `[6]`。这次，React将重新应用 effect，因为 `5 !== 6`。如果数组中有多个项，即使其中只有一个不同，React 也会重新应用 effect。
...
**重要：**如果使用此优化，请确保数组包含组件范围中随时间变化且被 effect 使用的所有值（例如 props 和 state）。否则，代码将引用前一次渲染时的过时值。

<!-- If you want to run an effect and clean it up only once (on mount and unmount), use `React.useEffect0`. -->
如果你想只在 mount 时运行一次 effect，然后在 unmount 时进行清理，使用 `React.useEffect0`。
...
如果你对更多有关性能优化的主题感兴趣，请参考 [ReactJS 性能优化文档](https://reactjs.org/docs/hooks-faq.html#performance-optimizations)。
