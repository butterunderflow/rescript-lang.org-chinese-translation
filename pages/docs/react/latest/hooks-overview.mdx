...
在前面的章节中，我们学到了 React 组件是一个用于表示 UI 的基于特定 props 值的简单函数。为了使应用具有可用性，我们仍然需要一种通过用户输入或向服务器发送请求来加载数据，从而交互式地操作 props 的方法。
...
这就是 Hooks 的用武之地。Hook 是一种函数，它让我们可以为组件引入状态，并为不同的任务触发副作用，例如 HTTP 请求、直接访问 HTML DOM、获取窗口大小等等。
...
让我们看一个例子，下面是一个 `Counter` 组件，它允许用户通过点击按钮来增加 `count` 值，并且新的 `count` 值会在每次按钮点击时被渲染：
...
我们使用了 `React.useState` Hook。我们在组件函数中调用它来为组件添加局部状态。React 会在重渲染之间保留这些状态。`React.useState` 返回一个元组，包括当前状态值（`count`） 和一个更新状态值的函数（`setCount`）。你能在处理事件时调用更新函数，或将更新函数传递给其他组件。
...
这只是 Hook 应用的一个小例子。我们会在独立的 [useState](./hooks-state) 章节中介绍更多细节。
...
- [useReducer](./hooks-reducer): `useState` 的替代，使用state / action / reduce 模式
...
Hooks本质上是些简单函数，但是你需要在使用时遵循 _两条规则_。ReScript 不会在编译器中强制执行这些规则，如果你想要强制执行正确的 Hooks 约定，可以使用 [eslint-plugin](https://www.npmjs.com/package/eslint-plugin-react-hooks) 来检查编译的 JS 输出。
